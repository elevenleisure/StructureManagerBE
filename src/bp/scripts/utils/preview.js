...
export const playerHotbar = new Map();
export function tryDrawParticle(source, location) {
    try {
        source.spawnParticle(particleId, location);
    }
    catch (e) { }
}
export function drawBorder(source, AABB) {
    getEdgeIntegerPoints(AABB).forEach(v => {
        tryDrawParticle(source, v);
    });
}
export function startSelectSave(player, defaultPos1, defaultPos2) {
    saveHotbar(player);
    clearHotbar(player);
    let pos1 = defaultPos1;
    let pos2 = defaultPos2;
    ...
    function draw() {
        if (pos1 && pos2)
            drawBorder(player, createPreviewAABB(pos1, pos2));
    }
    function drawTimeout() {
        draw();
        if (shouldDraw)
            system.runTimeout(drawTimeout, 20);
    }
...
export function startSelectPlace(player, size, defaultPos, defaultStructureRotationIndex) {
    saveHotbar(player);
    clearHotbar(player);
    let placeLocation = defaultPos;
    const structureRotationTool = new StructureRotationTool(player);
    if (defaultStructureRotationIndex)
        structureRotationTool.currentSelection = defaultStructureRotationIndex;
    const vertexSelectionTool = new VertexSelectionTool(player);
