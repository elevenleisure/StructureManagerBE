...
export function showBrowse(player) {
    const structureManager = world.structureManager;
    const structureComponentGroupMap = {};
    ...
    dialog.add(search, new DialogSpacer(), new DialogButton("新建结构", () => showCreate(player), true), new DialogDivider(), new DialogSpacer());
    getAllStructureId().forEach(structureId => {
        ...
        const group = new DialogComponentGroup(new DialogLabel(` §b${path}`), new DialogSpacer(), new DialogLabel(` 命名空间：§d${namespace}`), new DialogSpacer(), new DialogLabel(` 方块数量：§2${structure.size.x * structure.size.y * structure.size.z}`), new DialogSpacer(), btn, new DialogDivider());
        structureComponentGroupMap[structureId] = group;
        dialog.add(group);
    });
    dialog.closeButton().show();
}
export function showStructureAction(player, sourceStructure) {
    ...
    if (!structure || !structure.isValid) {
        dialog.add(new DialogLabel("无效的结构ID！")).setOnClose(() => showBrowse(player)).closeButton().show();
        return;
    }
    const { namespace, path } = praseStructureId(structure.id);
    dialog.add(new DialogLabel(` §b${path}`), new DialogSpacer(), new DialogLabel(` 命名空间：§d${namespace}`), new DialogSpacer(), new DialogLabel(`尺寸：§c${structure.size.x}§rx§a${structure.size.y}§rx§9${structure.size.z}§r （共计${structure.size.x * structure.size.y * structure.size.z}个方块）`), new DialogSpacer(), new DialogDivider(), new DialogButton("放置", () => showPlace(player, structure), true), new DialogButton("重命名", () => showRenameStructure(player, structure), true), new DialogButton("复制", () => showCloneStructure(player, structure), true), new DialogButton("删除", () => showDeleteStructure(player, structure), true)).setOnClose(() => showBrowse(player)).closeButton().show();
}
function showRenameStructure(player, structure) {
    const dialog = new Dialog(player, "重命名");
    const newStructureIdTextField = new DialogTextField("新的结构ID", structure.id);
    dialog.add(newStructureIdTextField, new DialogSpacer(), new DialogButton("重命名", () => {
        try {
    ...
    }, true)).setOnClose((reason) => showStructureAction(player, reason === DataDrivenScreenClosedReason.ServerClosed ? newStructureIdTextField.text : structure)).closeButton().show();
}
function showCloneStructure(player, structure) {
    ...
    dialog.add(newStructureIdTextField, saveToWorldDropdown, new DialogSpacer(), new DialogButton("复制", () => {
        try {
...
function showDeleteStructure(player, structure) {
    const dialog = new Dialog(player, "删除？");
    dialog.add(new DialogLabel(`你确定要删除结构 ${structure.id} 吗？`), new DialogSpacer(), new DialogButton("确定", () => {
        try {
            deleteStructure(structure);
        }
        catch (e) {
            showError(player, e.message, () => showStructureAction(player, structure));
        }
    }, true)).setOnClose(() => showStructureAction(player, structure)).closeButton().show();
}
export function showError(player, errorMessage, callback) {
