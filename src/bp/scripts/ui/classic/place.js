...
export function showPlaceForm(player, structure) {
...
function showPlaceUserInput(player, data) {
    ...
    async function next(response) {
        ...
        try {
            ...
            if (!isFinite(placeLocationx))
                throw new Error("放置位置x必须是一个有效的数字。");
            const placeLocationy = Number(formValues[2]);
            if (!isFinite(placeLocationy))
                throw new Error("放置位置y必须是一个有效的数字。");
            const placeLocationz = Number(formValues[3]);
            if (!isFinite(placeLocationz))
                throw new Error("放置位置z必须是一个有效的数字。");
            data.placeLocation = { x: placeLocationx, y: placeLocationy, z: placeLocationz };
            data.rotation = StructureRotations[formValues[4]].rotation;
            showCreateForm2(player, data);
        }
        catch (e) {
...
function showCreateForm2(player, data) {
    ...
    async function next(response) {
        ...
        try {
            const formValues = response.formValues;
            data.mirror = StructureMirrorAxes[formValues[1]].mirrorAxis;
            data.animationMode = StructureAnimationModes[formValues[2]].animationMode;
            const animationSeconds = Number(formValues[3]);
            if (!isFinite(animationSeconds))
                throw new Error("动画秒数必须是一个有效的数字。");
            data.animationSeconds = animationSeconds;
            data.integrity = formValues[4];
            data.integritySeed = formValues[5].trim() === "" ? undefined : formValues[5];
            data.includeBlocks = formValues[6];
            data.includeEntities = formValues[7];
            data.waterlogged = formValues[8];
...
function showCreateForm3(player, data) {
    try {
        loadStructure(data.structure.id, player.dimension, data.placeLocation, data.animationMode, data.animationSeconds, data.includeBlocks, data.includeEntities, data.integrity / 100, data.integritySeed, data.mirror, data.rotation, data.waterlogged);
        new ActionFormData()
            .title("放置结构 - 3/3")
            .body("你的结构已经放置。")
            .button("确定")
