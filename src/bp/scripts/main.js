...
system.beforeEvents.startup.subscribe(event => {
    ...
    event.customCommandRegistry.registerCommand(openGuiCommand, origin => {
        ...
        system.run(() => {
            if (getPlayerConfigOr(origin.sourceEntity, Config.useNewForm, false)) {
                showBrowse(origin.sourceEntity);
            }
            else {
                showBrowseForm(origin.sourceEntity);
            }
        });
        return {
    ...
    event.customCommandRegistry.registerCommand(configCommand, (origin, config, value) => {
        if (!(origin.sourceEntity && origin.sourceEntity instanceof Player)) {
            return {
                status: CustomCommandStatus.Failure,
                message: "执行者必须是一个有效的玩家"
